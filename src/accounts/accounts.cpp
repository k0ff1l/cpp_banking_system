...
// TODO(fleurs) : on deposit check if >0;
// TODO(fleurs) : on withdrawal check if money available >= withdrawing money;
...
class user_db {
  ...
  class user {
   public:
    ...
    user(unsigned int money, std::string &name)
        : money_(money), name_(std::move(name)) {
      id_ = user_db::prev_id_ + 1;
      user_db::prev_id_ = id_;
  ...
  static std::unordered_map<unsigned int, user>
      all_users;  // key: id, pred: user
